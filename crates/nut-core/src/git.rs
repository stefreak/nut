//! Git operations for nut.
//!
//! Provides functions for cloning repositories and checking status.
...
pub struct RepoStatus {
    /// Relative path to the repository from the workspace root.
    pub path_relative: OsString,
    /// Whether the repository has any uncommitted changes.
    pub has_changes: bool,
    /// Number of modified files with unstaged changes.
...
///
/// Uses a local cache to speed up cloning. If the repository already exists
/// in the workspace, it will be updated if needed.
pub async fn clone(
    workspace_dir: &Path,
    full_name: &str,
...
pub fn find_repositories(workspace_dir: &Path) -> Result<Vec<PathBuf>> {
...
/// Get the status of all repositories in a workspace.
