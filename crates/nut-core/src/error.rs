//! Error types for nut-core library.
//!
//! These error types are designed to provide rich diagnostic information
//! for CLI error display via miette.
...
/// Core library errors.
///
/// These errors provide diagnostic information for rich CLI error display.
#[derive(Error, Debug, Diagnostic)]
pub enum NutError {
    #[error("Already in workspace")]
    #[diagnostic(
        code(nut::workspace::already_entered),
        help("Exit the current workspace before creating or entering a new one (for example, return to the home directory by running 'cd ~')")
    )]
    AlreadyInWorkspace,

    #[error("Not in a workspace. Current working directory: {working_directory}, Data directory: {data_directory}")]
    #[diagnostic(
        code(nut::workspace::not_entered),
        help("Create a new workspace with 'nut create' or enter one with 'nut enter <id>'. You need to be inside the workspace directory or pass the workspace ID via the --workspace option.")
    )]
    NotInWorkspace {
    ...
    #[error("Please provide either a query using --query or positional repository arguments, but not both.")]
    #[diagnostic(
        code(nut::args::query_and_positional_conflict),
        help("Use --query <query> OR positional arguments <owner>/<repo>, but not both at the same time")
    )]
    ...
    #[diagnostic(
        ...
        help("Use --query <query> to search for repositories or provide positional arguments <owner>/<repo>")
    )]
    InvalidArgumentCombination,

    #[error("Invalid full repository name: '{name}'.")]
    #[diagnostic(
        code(nut::args::invalid_repo_name),
    ...
    #[diagnostic(code(nut::apply::script_not_executable))]
    ...
    #[diagnostic(code(nut::workspace::not_found))]
    WorkspaceNotFound { id: String },
}

/// Result type for nut-core operations.
